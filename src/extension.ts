...
function findCellAboveCursor(
    ...
    for (let iLine = startLine; iLine > 0; iLine--){
        ...
        if (found){
            let level = found[1].replace('\t', '....').length;
...
export function activate(context: vscode.ExtensionContext) {
    ...
    async function runCell(isNext: boolean){
        ...
        const cellAbove = findCellAboveCursor(undefined);
        ...
        const cellBelow = findCellBelowCursor(cellLevel);
    ...
    // TODO: add
    //  - moveToPreviousCell(), moveToNextCell()
